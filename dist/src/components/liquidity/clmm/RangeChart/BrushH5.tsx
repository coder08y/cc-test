    ...
    useEffect(() => {
      ...
      brushBehavior.current = brushX<SVGGElement>()
        ...
        .filter(event => {
          ...
          // 只允许手柄区域的交互，阻止选中区域和空白区域的交互
          const target = event.target as SVGElement
          if (!target) {
            return false
          }

          // 检查目标元素或其父元素是否在 handle 区域内
          // 如果点击的是 handle 或其子元素，允许交互
          if (target.closest && target.closest('.handle')) {
            return true
          }
          ...
          // 检查父元素是否是 handle
          let currentElement: Element | null = target.parentElement
          while (currentElement) {
            if (currentElement.classList && currentElement.classList.contains('handle')) {
              return true
            }
            currentElement = currentElement.parentElement
          }

          // 其他所有情况都阻止（包括 selection 和 overlay 空白区域）
          return false
        })
        .on('brush', (event: D3BrushEvent<unknown>) => {
          ...
          // 只允许通过手柄拖动，不允许拖动选中区域
          // mode 为 'handle' 表示拖动手柄，'space' 和 'center' 表示拖动选中区域，应该被忽略
          if (mode !== 'handle') {
            return
          }
      ...
      select(brushRef.current)
        .selectAll('.selection')
        .attr('stroke', 'none')
        .attr('fill', 'rgba(118, 200, 255, 0.4)')
        .attr('cursor', 'default')
        .style('pointer-events', 'none')

      // 设置 overlay 的默认光标，但保留交互以便手柄可以工作（通过 filter 函数控制）
      select(brushRef.current).selectAll('.overlay').attr('cursor', 'default')

      // 确保手柄区域可以交互并显示正确的光标
      select(brushRef.current).selectAll('.handle').attr('cursor', 'ew-resize')
