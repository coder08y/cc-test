...
type AssetsInfoModalProps = {
  ...
  onConfirm: (amount: string, tokenInfo?: Token, isBaseAsset?: boolean, actionMode?: 'Deposit' | 'Withdraw') => void | Promise<void>
  isBaseAsset: boolean
...
export default function AssetsInfoModal({
  ...
  // 当前操作模式：支持在 Deposit 和 Withdraw 之间切换
  // 如果 title 是 'Deposit' 或 'Withdraw'，允许切换；否则使用 title 作为固定值
  const [currentActionMode, setCurrentActionMode] = useState<'Deposit' | 'Withdraw' | 'Repay' | 'Initialize & Deposit Collateral'>(title)
  const currentTitle = currentActionMode

  // 是否允许切换 Deposit/Withdraw（仅在 title 是 'Deposit' 或 'Withdraw' 时允许）
  const canSwitchDepositWithdraw = useMemo(() => {
    return title === 'Deposit' || title === 'Withdraw'
  }, [title])
  ...
  const pool = useDeepBookStore(state => {
    return state.pool
  })
  console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:101 ~ AssetsInfoModal ~ pool:', pool)
  const { currentAccount } = useAccountStore()

  // 确定使用的池子地址：优先使用传入的 pool，否则使用 currentDeepBookPool
  const targetPoolAddress = useMemo(() => {
    console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:108 ~ AssetsInfoModal ~ pool?.address:', pool?.address)
    return pool?.address || currentDeepBookPool?.address
  }, [pool?.address, currentDeepBookPool?.address])

  // 根据 type 选择使用哪个 hook 获取余额
  const spotBalance = useGetDeepBookBalance(targetPoolAddress)
  console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:111 ~ AssetsInfoModal ~ spotBalance:', spotBalance)

  // 确定使用的池子对象：优先使用传入的 pool，否则使用 currentDeepBookPool
  const targetPool = useMemo(() => {
    return pool || currentDeepBookPool
  }, [pool, currentDeepBookPool])

  // 从 store 读取 margin balance 数据（如果是 margin 模式）
  const marginBalanceData = useMarginStore((state: any) => {
    if (mode !== 'margin' || !currentAccount?.address || !targetPoolAddress) {
      return null
    }
    return state.getMarginBalanceData(currentAccount.address, targetPoolAddress)
  })

  // 保留 hook 调用以获取 baseBalance 和 quoteBalance 对象（store 中没有存储这些对象）
  const marginBalanceHook = useGetDeepBookMarginBalance(undefined, targetPoolAddress)
  ...
  // 使用 targetPoolAddress（优先使用传入的 pool，否则使用 currentDeepBookPool）
  const debtData = useDeepbookMarginDebt(targetPoolAddress)
  ...
  const {
    ...
    totalAssetsValue: baseTotalAssetsValue,
    totalDebtValue: currentTotalDebtValue,
    riskRatio
  } = useCalculateRiskRatio(pool?.address)

  // 获取价格（用于计算最大可提取金额）
  const { basePrice, quotePrice } = useDeepBookMarginPrices(pool?.address)
  ...
  // 根据 currentActionMode 动态计算 currentTotalAssetsValue
  // 当切换 Deposit/Withdraw 模式时，使用 calculateRiskRatio 重新计算当前值
  const [dynamicTotalAssetsValue, setDynamicTotalAssetsValue] = useState<string | null>(null)

  useEffect(() => {
    if (mode !== 'margin' || !calculateRiskRatio) {
      setDynamicTotalAssetsValue(null)
      return
    }

    const updateTotalAssetsValue = async () => {
      try {
        // 根据 currentActionMode 确定 action
        let action: 'deposit' | 'withdraw' | undefined
        if (currentActionMode === 'Deposit') {
          action = 'deposit'
        } else if (currentActionMode === 'Withdraw') {
          action = 'withdraw'
        }

        // 如果没有 action（Repay 或其他），使用基础值
        if (!action) {
          setDynamicTotalAssetsValue(null)
          return
        }

        // 调用 calculateRiskRatio 获取当前值（不传 amount，只传 action）
        const result = await calculateRiskRatio({ action })
        setDynamicTotalAssetsValue(result.totalAssetsValue)
      } catch (error) {
        console.error('Error updating total assets value:', error)
        setDynamicTotalAssetsValue(null)
      }
    }

    updateTotalAssetsValue()
  }, [currentActionMode, mode, calculateRiskRatio])

  // 使用动态值（如果存在），否则使用基础值
  const currentTotalAssetsValue = dynamicTotalAssetsValue !== null ? dynamicTotalAssetsValue : baseTotalAssetsValue

  // 判断当前 token 的类型：base、quote 或 DEEP（第三种情况）
  const tokenType = useMemo(() => {
    if (!currentToken?.coin_type || !targetPool) return null
    const isBase = currentToken.coin_type === targetPool.baseAssets?.coin_type
    const isQuote = currentToken.coin_type === targetPool.quoteAssets?.coin_type
    if (isBase) return 'base'
    if (isQuote) return 'quote'
    return 'deep' // 既不是 base 也不是 quote，是 DEEP（第三种情况）
  }, [currentToken?.coin_type, targetPool])
  ...
  useEffect(() => {
    ...
    const calculateValues = async () => {
      try {
        ...
        console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:214 ~ calculateValues ~ currentActionMode:', currentActionMode)
        if (currentTitle === 'Deposit' || currentActionMode === 'Deposit') {
          action = 'deposit'
        } else if (currentTitle === 'Withdraw' || currentActionMode === 'Withdraw') {
          action = 'withdraw'
        } else if (currentTitle === 'Repay') {
          action = 'repay'
        }
        console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:221 ~ calculateValues ~ action:', action)
        ...
        // 获取当前值（不传参数）
        const currentResult = await calculateRiskRatio({ action })
        ...
        console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:240 ~ calculateValues ~ afterResult:', afterResult)
        ...
        const afterTotalAssetsValue =
          (action === 'deposit' || action === 'withdraw') && d(afterResult.totalAssetsValue || '0').lt(0) ? '0' : afterResult.totalAssetsValue
        ...
        console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:264 ~ calculateValues ~ afterCollateralValue:', afterCollateralValue)
    ...
    calculateValues()
  }, [inputValue, currentTitle, currentIsBaseAsset, tokenType, mode, calculateRiskRatio, currentActionMode])

  // 当 token、isBaseAsset 或 title 变化时，更新当前选中的 token 和操作模式
  useEffect(() => {
    if (isOpen) {
      setCurrentToken(token)
      setCurrentIsBaseAsset(isBaseAsset)
      // 如果允许切换，将 currentActionMode 设置为 title；否则保持当前值
      if (canSwitchDepositWithdraw) {
        setCurrentActionMode(title)
      } else {
        setCurrentActionMode(title)
      }
      // 重置输入值
  ...
  }, [token, isBaseAsset, isOpen, title, canSwitchDepositWithdraw])

  // 处理 Deposit/Withdraw 模式切换
  const handleActionModeChange = useCallback(
    (mode: 'Deposit' | 'Withdraw') => {
      if (!canSwitchDepositWithdraw) return
  ...
  const switchableTokenList = useMemo(() => {
    if (!canSwitchToken || !targetPool) return []
    const tokens: Token[] = []
    if (targetPool.baseAssets) {
      tokens.push(targetPool.baseAssets)
    }
    if (targetPool.quoteAssets) {
      tokens.push(targetPool.quoteAssets)
    }

    // console.log('🚀🚀🚀 ~ switchableTokenList ~ tokens:', tokens)

    return tokens
  }, [canSwitchToken, targetPool])
  ...
  const handleTokenChange = useCallback(
    (newToken: Token) => {
      ...
      const isNewBaseAsset = targetPool?.baseAssets?.coin_type === newToken?.coin_type
  ...
  const maxWithdrawableAmount = useMemo(() => {
    ...
    try {
      const minWithdrawRiskRatio = marginData.minWithdrawRatio || targetPool?.minWithdrawRiskRatio
      ...
      if (currentIsBaseAsset) {
        ...
        const isDeepToken =
          ...
          currentToken.coin_type !== targetPool?.baseAssets?.coin_type &&
          currentToken.coin_type !== targetPool?.quoteAssets?.coin_type
    ...
    currentTitle,
    marginData,
    targetPool?.minWithdrawRiskRatio,
    ...
    deepFreeBalance,
    currentToken?.coin_type,
    targetPool?.baseAssets?.coin_type,
    targetPool?.quoteAssets?.coin_type
  ])
  ...
  const currentBalance = useMemo(() => {
    ...
    if (currentTitle === 'Deposit') {
      console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:443 ~ AssetsInfoModal ~ baseBalance:', baseBalance)
      if (tokenType === 'base') {
        return baseBalance?.balanceFormat
      } else if (tokenType === 'quote') {
        return quoteBalance?.balanceFormat
  ...
  useEffect(() => {
    if (inputSource === 'input') {
      if (!inputValue || inputValue === '0') {
        ...
        if (currentTitle === 'Repay' && mode === 'margin') {
          const debtAmount = currentIsBaseAsset ? baseDebt : quoteDebt
          console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:604 ~ AssetsInfoModal ~ debtAmount:', debtAmount)
          const availableBalance = currentBalance || '0'
  ...
  const btnInfo = useMemo(() => {
    if (
      (d(riskRatio).lt(targetPool?.minWithdrawRiskRatio) ||
        (calculatedHealthFactorAfter && d(calculatedHealthFactorAfter).lt(targetPool?.minWithdrawRiskRatio))) &&
      currentTitle == 'Withdraw' &&
      tokenType === 'deep' &&
      tradeType == 'Margin'
    ) {
      return {
  ...
  const collateralValueDisplay = useMemo(() => {
    ...
    if (currentTotalAssetsValue) {
      console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:834 ~ AssetsInfoModal ~ currentTotalAssetsValue:', currentTotalAssetsValue)
  ...
  const collateralAfterValueDisplay = useMemo(() => {
    ...
    console.log('🚀🚀🚀 ~ AssetsInfoModal.tsx:858 ~ AssetsInfoModal ~ marginData.totalCollateralAfter:', marginData.totalCollateralAfter)
  ...
  const renderMarginLeftPanel = () => {
    ...
    const {
      healthFactor = 2.5,
      // liquidationRatio = targetPool?.liquidationRiskRatio,
      minBorrowRatio = targetPool?.minBorrowRiskRatio,
      minWithdrawRatio = targetPool?.minWithdrawRiskRatio
      // totalCollateral,
      // totalDebt,
      // borrowLimit,
      ...
      <Button
        ...
        onClick={async () => {
          ...
          try {
            ...
            // 只在 Deposit 或 Withdraw 模式下传递 actionMode
            const actionModeForConfirm = currentActionMode === 'Deposit' || currentActionMode === 'Withdraw' ? currentActionMode : undefined
            await onConfirm(inputValue, currentToken, currentIsBaseAsset, actionModeForConfirm)
            if (isOpen) {
              setTimeout(() => {
                setIsSubmitting(false)
              }, 100)
  ...
  if (isApp) {
    return (
      <VaulDrawer isOpen={isOpen} onClose={() => onClose(false)} placement="bottom" padding="16px">
        {canSwitchDepositWithdraw ? (
          <VStack w="100%" alignItems="stretch" mb="12px" gap="12px">
            <SelectTab
              type="outlineTab"
              tabList={[{ label: 'Deposit' }, { label: 'Withdraw' }] as Tab[]}
              currentTab={currentActionMode === 'Deposit' ? 'Deposit' : 'Withdraw'}
              handleChangeTab={tab => {
                const mode = typeof tab.label === 'string' ? tab.label : 'Deposit'
                handleActionModeChange(mode === 'Deposit' ? 'Deposit' : 'Withdraw')
              }}
              itemStyle={{
                fontSize: '14px',
                fontWeight: '500',
