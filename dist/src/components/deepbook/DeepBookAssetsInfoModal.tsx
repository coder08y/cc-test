...
export default function DeepBookAssetsInfoModal() {
  ...
  const pool = useDeepBookStore(state => state.pool)
  ...
  const { riskRatio: currentHealthFactor } = useCalculateRiskRatio(pool?.address)
  ...
  const marginBalanceData = useMarginStore((state: any) => {
    ...
    return state.getMarginBalanceData(currentAccount.address, pool?.address || currentDeepBookPool.address)
  })
  const marginDebtData = useDeepbookMarginDebt(pool?.address || currentDeepBookPool.address)
  ...
  return (
    <AssetsInfoModal
      ...
      isBaseAsset={currentTokenInfo?.symbol === pool?.baseAssets?.symbol || currentTokenInfo?.symbol === pool?.baseAssets?.symbol}
      ...
      onConfirm={async (inputValue, tokenInfoParam, isBaseAssetParam, actionMode) => {
        // 使用传入的参数，如果没有则使用默认值
        ...
        // 统一使用 actionMode（如果存在，优先使用弹窗内切换后的值），否则使用 actionType（初始值）
        const finalActionMode = actionMode || actionType
        ...
        try {
          ...
          } else if (finalActionMode === 'Deposit') {
            if (isMarginPool) {
          ...
          } else if (finalActionMode === 'Withdraw') {
            if (isMarginPool) {
