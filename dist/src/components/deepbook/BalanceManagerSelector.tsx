import { fetchSingleMarginManagerBalance } from '@/hooks/deepbook/margin/useGetDeepBookMarginBalance'
...
export default function BalanceManagerSelector({ isMarginPool = false, mb = '0' }: BalanceManagerSelectorProps) {
  ...
  const checkMarginManagerHasBalance = useMemo(() => {
    return (managerId: string) => {
      const cached = marginManagerBalanceCache[managerId]
      if (!cached) return false
      return d(cached.base).gt(0) || d(cached.quote).gt(0)
    }
  }, [marginManagerBalanceCache])
  ...
  useEffect(() => {
    ...
    fetchMarginBalances()
  }, [isMarginPool, currentAccount?.address, currentDeepBookPool?.address, marginManagerList, deepBookSDK])
        ...
        <>
          ...
          <MenuList p="8px" w="260px" borderRadius="8px" maxH="280px" overflowY="scroll">
            {managerList?.map((item: any, index: number) => {
              ...
              const hasBalance = isMarginPool
                ? checkMarginManagerHasBalance(item?.margin_manager_id || '')
                : checkManagerHasFreeBalance(item.balanceManager)
              return (
                <MenuItem
                ...
                >
                  <HStack gap="4px">
                    <AddressCopyLink
                      ...
                      addressClickDisabled={true}
                      color="text_caption"
                      onClickLink={() => {
                        window.open(getExplorerUrl(displayAddress, 'account'))
                      }}
