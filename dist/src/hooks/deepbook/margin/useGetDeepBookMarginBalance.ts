...
export async function fetchSingleMarginManagerBalance(
  ...
  try {
    ...
    const res = await marginUtils.getPoolMarginManagerBalance(params).catch(() => {})
    const baseBalanceRaw = res?.[baseCoinType] || '0'
    const quoteBalanceRaw = res?.[quoteCoinType] || '0'
    const deepBalanceRaw = res?.[DeepCoin?.coin_type] || '0'
...
export default function useGetDeepBookMarginBalance(network: 'testnet' | 'mainnet' = 'testnet', managerId?: string | null) {
  ...
  const fetchMarginBalances = useCallback(
    async (managerIdOverride?: string | null) => {
      ...
      try {
        ...
        console.log('0127###fetchSingleMarginManagerBalance 触发1111')
        const {
          base: baseBalanceFormatted,
          quote: quoteBalanceFormatted,
          deep: deepBalanceFormatted
  ...
  useEffect(() => {
    ...
    if (prevManagerIdRef.current !== null && prevManagerIdRef.current !== marginManagerId) {
      // 清除旧的余额数据，确保显示新 manager 的数据
      const store = useMarginStore.getState()
      const oldManagerId = prevManagerIdRef.current || ''
      store.setMarginBalanceData(currentAccountAddress, currentDeepBookPoolAddress, oldManagerId, {
        baseFreeBalance: '0',
        quoteFreeBalance: '0',
        baseLockedBalance: '0',
