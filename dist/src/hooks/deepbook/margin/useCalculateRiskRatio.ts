...
export function useCalculateRiskRatio(poolAddress?: string) {
  const { currentDeepBookPool } = useDeepBookStore()
  const { currentAccount } = useAccountStore()
  const { getMarginManagerId } = useMarginOrderUtils()
  const { baseDebt, quoteDebt, baseDebtUSD, quoteDebtUSD, totalDebtValue: totalDebtValueFromStore } = useDeepbookMarginDebt(poolAddress)
  const { basePrice, quotePrice } = useDeepBookMarginPrices(poolAddress)
  ...
  const balanceData = useMarginStore((state: any) => {
    ...
    return state.getMarginBalanceData(currentAccount.address, poolAddress || currentDeepBookPool.address)
  })
  ...
  const storeRiskRatio = useMarginStore((state: any) => {
    if (!currentAccount?.address || !poolAddress || !currentDeepBookPool?.address) {
      return null
    }
    return state.getRiskRatio(currentAccount.address, poolAddress || currentDeepBookPool.address)
  })
  ...
  useEffect(() => {
    ...
    if (prevAccount !== undefined && prevAccount !== currentAccountAddress) {
      ...
      if (poolAddress || (currentDeepBookPool?.address && prevAccount)) {
        store.setRiskRatio(prevAccount, poolAddress || currentDeepBookPool.address, {
          riskRatio: '0',
          totalAssetsValue: '0',
          totalDebtValue: '0',
          baseAssetValue: '0',
  ...
  }, [currentAccount?.address, poolAddress, currentDeepBookPool?.address])
  ...
  useEffect(() => {
    ...
    // 如果正在计算中，跳过
    if (store.isRiskRatioCalculating(currentAccount.address, poolAddress || currentDeepBookPool.address)) {
      return
    }
    ...
    if (
      ...
      store.setRiskRatio(currentAccount.address, poolAddress || currentDeepBookPool.address, {
        riskRatio: '0',
        totalAssetsValue: '0',
        totalDebtValue: '0',
        baseAssetValue: '0',
    ...
    const updateRiskRatio = async () => {
      ...
      // 标记正在计算中
      store.setRiskRatioCalculating(currentAccount.address, poolAddress || currentDeepBookPool.address, true)

      try {
        if (calculateRiskRatioRef.current) {
          const result = await calculateRiskRatioRef.current()
          console.log('🚀🚀🚀 ~ useCalculateRiskRatio.ts:439 ~ updateRiskRatio ~ result:', result)
          // console.log('useCalculateRiskRatio: 计算完成', result)
          // 更新 store
          store.setRiskRatio(currentAccount.address, poolAddress || currentDeepBookPool.address, result)
        } else {
        ...
        // 重置计算状态
        store.setRiskRatioCalculating(currentAccount.address, poolAddress || currentDeepBookPool.address, false)
      }
    }
