...
export default function useMarginLockedBalance() {
  ...
  const fetchMarginOpenOrders = useCallback(async () => {
    ...
    if (
      ...
      console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:138 ~ useMarginLockedBalance ~ setMarginOpenOrders:')
      setMarginOpenOrders([])
      return
    }

    // 获取 margin manager ID（优先使用用户选择的 manager，但需要验证它是否属于当前池子）
    // 从 store 获取最新的用户选择信息
    const latestSelectedManagerInfo = marginStore.getCurrentMarginManagerInfo(currentAccountAddress)

    let marginManager: any = null

    // 优先使用用户选择的 manager，但需要验证它是否属于当前池子
    if (latestSelectedManagerInfo?.margin_manager_id && currentPool.address) {
      const selectedManager = (latestMarginManagerByAccount as any[]).find(
        (m: any) => m?.margin_manager_id === latestSelectedManagerInfo.margin_manager_id && m?.deepbook_pool_id === currentPool.address
      )
      if (selectedManager) {
    ...
    if (!marginManager) {
      marginManager = (latestMarginManagerByAccount as any[]).find((m: any) => m.deepbook_pool_id === currentPool.address)
    }

    if (!marginManager?.margin_manager_id) {
      console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:164 ~ useMarginLockedBalance ~ setMarginOpenOrders:')
      setMarginOpenOrders([])
      return
    }
    ...
    try {
      ...
      if (!currentPool?.baseAssets?.coin_type || !currentPool?.quoteAssets?.coin_type) {
        console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:206 ~ useMarginLockedBalance ~ setMarginOpenOrders:')
        setMarginOpenOrders([])
        setLockedOrdersFetching(finalRequestKey, false)
        setIsLoading(false)
        return
      ...
      if (!poolInfo || !poolInfo.baseCoin?.coinType || !poolInfo.quoteCoin?.coinType) {
        console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:225 ~ useMarginLockedBalance ~ setMarginOpenOrders:')
        setMarginOpenOrders([])
        setLockedOrdersFetching(finalRequestKey, false)
        setIsLoading(false)
        return
      ...
      if (!marginManager.margin_manager_id || typeof marginManager.margin_manager_id !== 'string') {
        console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:234 ~ useMarginLockedBalance ~ setMarginOpenOrders:')
        setMarginOpenOrders([])
        setLockedOrdersFetching(finalRequestKey, false)
        setIsLoading(false)
        return
      ...
      if (!deepBookSDK?.MarginUtils || typeof deepBookSDK.MarginUtils.getAccountOpenOrders !== 'function') {
        console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:243 ~ useMarginLockedBalance ~ setMarginOpenOrders:')
        setMarginOpenOrders([])
        setLockedOrdersFetching(finalRequestKey, false)
        setIsLoading(false)
        return
      ...
      if (abortController.signal.aborted) {
        console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:258 ~ useMarginLockedBalance ~ setMarginOpenOrders:')
        return
      }
      ...
      if (!abortController.signal.aborted && currentRequestKey === finalRequestKey) {
        ...
        console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:290 ~ useMarginLockedBalance ~ setMarginOpenOrders:', wrappedOrders)
      }
    } catch (error: any) {
      // 如果请求被取消，不处理错误
      ...
      if (
        ...
        setMarginOpenOrders([])
        console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:312 ~ useMarginLockedBalance ~ setMarginOpenOrders:')
      }
    } finally {
      // 请求完成，重置标记（全局状态）
  ...
  const selectedManagerInfo = useMarginStore((state: any) => state.getCurrentMarginManagerInfo(currentAccountAddress))
  ...
  useEffect(() => {
    ...
    console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:416 ~ useMarginLockedBalance ~ shouldFetch:', shouldFetch)
    if (shouldFetch) {
      // 满足所有条件，触发获取订单
    ...
    selectedManagerInfo?.margin_manager_id
  ])
  ...
  const lockedBalance = useMemo(() => {
    ...
    let baseLock = '0'
    let quoteLock = '0'
    console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:496 ~ useMarginLockedBalance ~ marginOpenOrders:', marginOpenOrders)
  ...
  return useMemo(() => {
    console.log('🚀🚀🚀 ~ useMarginLockedBalance.ts:521 ~ useMarginLockedBalance ~ lockedBalance:', lockedBalance)
    const result = {
