...
export default function useMarginTrade() {
  ...
  const balanceData = useMarginStore((state: any) => {
    ...
    return state.getMarginBalanceData(currentAccount.address, currentDeepBookPool.address, currentBalanceManagerInfo?.margin_manager_id)
  })
  ...
  const available = useMemo(() => {
    ...
    if (tradeType === DeepBookPoolMarginTabs.Long) {
      ...
      console.log('🚀🚀🚀 ~ useMarginTrade.ts:380 ~ useMarginTrade ~ quoteFreeBalance:', quoteFreeBalance)
  ...
  const calculatedBorrowAmount = useMemo(() => {
    ...
    if (tradeType === DeepBookPoolMarginTabs.Long) {
      ...
      console.log(
        '🚀🚀🚀 ~ useMarginTrade.ts:638 ~ useMarginTrade ~ currentBaseFree:',
        currentBaseFree.toString(),
        orderTotalValue.toString(),
        amount,
        maxFee
      )
