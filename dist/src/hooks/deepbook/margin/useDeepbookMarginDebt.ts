...
export default function useDeepbookMarginDebt(managerId?: string | null) {
  ...
  const baseDebt = useMarginStore((state: any) => {
    if (!currentAccount?.address || !currentDeepBookPool?.address) return '0'
    const key = `${currentAccount.address}-${currentDeepBookPool.address}-${marginManagerId}`
  ...
  useEffect(() => {
    ...
    if (prevAccount !== undefined && prevAccount !== currentAccountAddress) {
      ...
      if (currentDeepBookPool?.address && prevAccount) {
        store.setMarginDebt(prevAccount, currentDeepBookPool.address, marginManagerId as string, {
          baseDebt: '0',
          quoteDebt: '0',
          baseDebtUSD: '0',
          quoteDebtUSD: '0',
      ...
      if (currentAccountAddress && currentDeepBookPool?.address) {
        const newAccountDebt = store.getMarginDebt(currentAccountAddress, currentDeepBookPool.address, marginManagerId as string)
        if (newAccountDebt.baseDebt !== '0' || newAccountDebt.quoteDebt !== '0') {
          store.setMarginDebt(currentAccountAddress, currentDeepBookPool.address, marginManagerId as string, {
            baseDebt: '0',
            quoteDebt: '0',
            baseDebtUSD: '0',
            quoteDebtUSD: '0',
  ...
  const fetchMarginDebt = useCallback(async () => {
    ...
    if (!pool?.isMarginPool) {
      store.setMarginDebt(accountAddress, pool.address, marginManagerId as string, {
        baseDebt: '0',
        quoteDebt: '0',
        baseDebtUSD: '0',
        quoteDebtUSD: '0',
    ...
    managerIdToUse = marginManagerId || marginManager?.margin_manager_id || null
  ...
  }, [getMarginUtils, marginManagerId])
  ...
  useEffect(() => {
    ...
    if (!marginManagerId && latestMarginManagerByAccount.length === 0) {
      const existingDebt = store.getMarginDebt(currentAccount.address, currentDeepBookPool.address, marginManagerId as string)
      if (existingDebt.baseDebt !== '0' || existingDebt.quoteDebt !== '0') {
        store.setMarginDebt(currentAccount.address, currentDeepBookPool.address, marginManagerId as string, {
          baseDebt: '0',
          quoteDebt: '0',
          baseDebtUSD: '0',
          quoteDebtUSD: '0',
  ...
  useEffect(() => {
    ...
    const currentDebt = store.getMarginDebt(currentAccount.address, currentDeepBookPool.address, marginManagerId as string)
