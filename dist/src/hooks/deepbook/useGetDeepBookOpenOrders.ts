  ...
  const getDeepBookOpenOrdersCombined = async (marginManagerByAccount: any, poolInfo = currentDeepBookPool, account?: string, requestId?: string) => {
    ...
    try {
      ...
      const marginAccount =
        ...
        (marginManagerByAccount as any[]).find((m: any) => m.deepbook_pool_id === poolInfo?.address)?.margin_manager_id
  ...
  const getDeepBookAllOpenOrdersBySdk = async (
    ...
    try {
      ...
      if (isMarginPool) {
        deepBookAccount =
          currentSelectedManagerInfo?.margin_manager_id ||
          (marginManagerByAccount as any[]).find((m: any) => m.deepbook_pool_id === currentDeepBookPool?.address)?.margin_manager_id
      } else {
        deepBookAccount = getCurrentBalanceManagerInfo(currentAccount?.address as string)?.balanceManager
        ...
        .map((item: any) => {
          return {
            ...
            margin_manager_id: (marginManagerByAccount as any[]).find(
              (m: any) =>
                m.deepbook_pool_id === item?.address &&
                m.owner === currentAccount?.address &&
                m.margin_manager_id === currentSelectedManagerInfo?.margin_manager_id
            )?.margin_manager_id
          }
        })
      ...
      console.log('getAccountAllMarketsOpenOrders:', currentAccount?.address, orderList)
      ...
      // 检查请求 ID 是否仍然匹配（防止竞态条件）
      console.log(
        '🚀🚀🚀 ~ useGetDeepBookOpenOrders.ts:583 ~ getDeepBookAllOpenOrdersBySdk ~ isCheckedAllMarketsRef.current && deepBookAccount && currentRequestIdRef.current === requestId:',
        isCheckedAllMarketsRef.current,
        deepBookAccount,
        currentRequestIdRef.current === requestId
      )
      if (isCheckedAllMarketsRef.current && (deepBookAccount || isMarginPool) && currentRequestIdRef.current === requestId) {
        console.log('pBookAllOpenOrdersBySdk ~ allList:', allList)
        setDeepBookOpenOrders(allList)
        // setOrderListLoading(false)
