...
function useDlmmAnalyticChart(poolInfo?: any) {
  ...
  const { getDlmmHistogramData } = useGetHistogramData()
  ...
  }, [queryPoolId, poolInfo])

  const dlmmApiPoolInfo = useMemo(() => {
  ...
  const fetchAnalyticsData = async () => {
    ...
    try {
      ...
      const result = await getDlmmHistogramData({
  ...
  useEffect(() => {
    fetchAnalyticsData()
  }, [poolId, currentChartTab, currentDateType])
  ...
  const defaultDisplay = useMemo(() => {
    if (currentChartTab === ChartsTabsEnum.volume) {
      return { value: dlmmApiPoolInfo?.volume24Display, title: 'Volume (24H)' }
    } else if (currentChartTab === ChartsTabsEnum.tvl) {
      return { value: dlmmApiPoolInfo?.tvlDisplay, title: 'TVL' }
    } else if (currentChartTab === ChartsTabsEnum.fees) {
      return { value: dlmmApiPoolInfo?.fees24Display, title: 'Fees (24H)' }
    }
  }, [currentChartTab, dlmmApiPoolInfo?.volume24Display, dlmmApiPoolInfo?.tvlDisplay, dlmmApiPoolInfo?.fees24Display])
...
export default useDlmmAnalyticChart
