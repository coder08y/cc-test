...
const store = (set: any, get: any): MarginStore => ({
  ...
  setMarginDebt: (
    ...
    const key = `${account}-${poolAddress}-${managerId}`
    set((state: MarginStore) => ({
      marginDebts: {
        ...state.marginDebts,
        [key]: {
  ...
  getMarginDebt: (account: string, poolAddress: string, managerId: string) => {
    const key = `${account}-${poolAddress}-${managerId}`
    const state = get()
    return (
...
const useMarginStore = create(
  persist(store, {
    ...
    partialize: (state: MarginStore) => ({
      ...
      // currentMarginManagerInfoMap: state.currentMarginManagerInfoMap,  // toDo: 会引发order首次重复请求，而且如果持久化这里可能有隐患， 比如用户下次带着其他池子地址刷新这里可能会先是上次池子的margin manager然后数据请求后再变为当前池子的margin manager，所以先注释
